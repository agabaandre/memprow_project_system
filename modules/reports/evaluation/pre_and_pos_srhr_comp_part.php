		...
		<form action="" method="POST" style="width:30%;" class="form-inline">
			<div id="">
				...
				<select class="form-control select2" name="field_activity_id" id="factivity_id myselect" style="width:100%;">
					<?php
					$sql2 = mysqli_query($dbcon, "SELECT DISTINCT field_activity_id, training FROM field_work where field_activity_id in (SELECT distinct training from field_participants)");
	...
	<div class="col-md-12">
		...
		<div id="printableArea">
			<div class="box-header with-border">
				<h5 class="box-title">Pre and Post SRHR Comparison by Participant</h5>
				<p class="danger"></p>
			</div>
			<table id="mydata" class="table table-hover table-responsive table-bordered">
				...
				<tbody>
					...
					$i = 1;
					$sql = "SELECT participants.surname,participants.firstname,participants.othername,participants.position,field_work.start_date,field_work.training,field_work.field_activity_id, SUM(q51+q52+q53+q53+q54+q55+q56) as
					...
					Q3postsrhr FROM me_f1, me_f2,participants,field_work where me_f1.training_id LIKE'%$field_activity_id' and me_f1.participant_id LIKE'%$participant_id' and me_f1.training_id=me_f2.training_id and me_f1.training_id=field_work.field_activity_id 
					and me_f2.training_id=field_work.field_activity_id and me_f1.participant_id=me_f2.participant_id and me_f1.participant_id=participants.participant_id
					and me_f2.participant_id=participants.participant_id GROUP BY me_f1.training_id ";
				...
				<script type="text/javascript">
					$(function() {
						$('#graph').highcharts({
							...
							title: {
								text: '<?php echo $myname;  ?> SRHR COMPARISON (PRE AND POST) Qn5 AND Qn2, Q6 and Q3 '
							},
							xAxis: {
								categories: ['Q5 Pre SRHR SCORE', 'Q2 POST SRHR SCORE', 'Q6 Pre SRHR SCORE', 'Q3 POST SRHR SCORE']
							},
							yAxis: {
								min: 0,
								title: {
							...
							series: [{
									name: '%Score Q5 and Q2, Q6 and Q3',
									color: '#b14383',
									data: [<?php echo (($q5preshr / 15) * 100); ?>, <?php echo (($q2post / 15) * 100); ?>, <?php echo (($q6pre / 6) * 100); ?>, <?php echo (($q3post / 6) * 100); ?>]
